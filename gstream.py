...
class SimVideoManager:
    def __init__(self, ip_dest="192.168.2.9", port=5000, width=640, height=480):
        ...
        pipeline_in = (
            ...
            "application/x-rtp, payload=96 ! rtpjitterbuffer ! rtph264depay ! "
            "avdec_h264 ! videoconvert ! video/x-raw, format=BGR ! appsink sync=false drop=true max-buffers=1"
        )
        ...
        pipeline_out = (
            ...
            f"rtph264pay ! udpsink host=host.docker.internal port=5601"
        )
        self.writer = cv2.VideoWriter(pipeline_out, cv2.CAP_GSTREAMER, 0, 15, (width, height), True)
